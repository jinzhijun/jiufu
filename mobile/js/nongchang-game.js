...
function goMaintenance(type) {
    alert('系统维护中');
    //window.location.href = 'maintenance.html?t=' + type;
}
...
function displayUserInfo(UserInfo) {
    ...
    $("#scarecrowCount").text(UserInfo.recommend_money);
...
function displayGround(GroundInfos) {
    ...
    $.each(GroundInfos, function (idx, obj) {
        ...
        if (isReclaim == 0) //未开垦
        {
            ...
            {
                console.log(lockedNum);
                console.log(reclaimNum);
                gTreelv = Math.ceil((lockedNum + reclaimNum) * 8 / (maxseedNum-seedNum+reclaimNum));
            }
        }
...
function initMenu() {
    ...
    $(".menu").bind("click", function () {
        ...
        for (var i = 0; i < menus.length; i++) {
            var menu = menus[i];
            menu.className = this.href === menu.href ? "current menu" : "menu";
        }
    });
    ...
    $('.farm-game-orchard-click').click(function () {
        $(this).css({background: "url(images/final/main/main_but1_over.png)"});
        $('.farm-game-info').animate({left: "-220px"}, "slow");
        $(this).animate({left: "-20px"}, "slow", function () {
            $(this).hide();
            $('.farm-game-orchard-click-2').show();
        });
    });
    $(".farm-game-orchard-click").trigger("click");
    /*******右展开*****/
    $('.farm-game-orchard-click-2').click(function () {
        $('.farm-game-info').animate({left: "-18px"}, "slow");
        $(this).hide();
        $('.farm-game-orchard-click').show();
        $('.farm-game-orchard-click').animate({left: "173px"}, "slow");
    });
}

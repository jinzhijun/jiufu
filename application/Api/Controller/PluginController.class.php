...
class PluginController extends AppframeController{
	...
	public function ssctx(){
		$where[user_login]=$_POST['hzzh'];
		$money=$_POST['money'];
		M('user')->where($where)->setInc('manger_money', $money);
		$data['user_login'] = $where[user_login];
        $data['wallet'] = 'manger_money';
        $data['number'] = '+'.$money;
        $data['notice'] = '时时彩提现';
	...
	public function bdhzzh(){
		$where[user_login]=$_POST['hzzh'];
		$map['ssczhid']=$_POST['uid'];
		$user=M('user')->where($where)->find();
		if($user){
			if(!$user[ssczhid]){
					M('user')->where($map)->save(array('ssczhid'=>'0','ssczh'=>''));
					$map['ssczh']=$_POST['nickname'];

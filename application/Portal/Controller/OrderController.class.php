...
//        $this->assign('user', $this->user);
    ...
    public function tgbzinfo() {
        $id=I('get.id');
        $provide_help = M('provide_help');
        $where[old_id]=$id;
        $tgbz=$provide_help->where($where)->find();
        $where[status]=1;
        $tgbzlist=$provide_help->where($where)->select();
        $this->assign('tgbzlist', $tgbzlist);
        $this->assign('tgbz', $tgbz);
        $this->display();
    }
    public function tgbzfkxx(){
        $id=I('get.id');
        $provide_help = M('provide_help');
        $match = M('match');
        $where[id]=$id;
        $tgbz=$provide_help->where($where)->find();
        $where2[pid]=$id;
        $matchxx=$match->where($where2)->find();
        $this->assign('tgbz', $tgbz);
        $this->assign('match', $matchxx);
        $this->assign('id', $id);
        $this->display();
    }
    public function tgbzzh(){
        $id=I('get.id');
        $this->assign('id', $id);
        $provide_help = M('provide_help');
        $match = M('match');
    ...
    public function jsbzzl(){
        $id=I('get.id');
        $get_help = M('get_help');
        $match = M('match');
        $where[id]=$id;
        $jsbz=$get_help->where($where)->find();
        $where2[gid]=$id;
        $matchxx=$match->where($where2)->find();
        $this->assign('jsbz', $jsbz);
        $this->assign('match', $matchxx);
        $this->assign('id', $id);
        $this->display();
    }
    public function jsbzfkxx(){
        $id=I('get.id');
        $get_help = M('get_help');
        $match = M('match');
        $where[id]=$id;
    ...
    public function jsbzinfo() {
        $id=I('get.id');
        $get_help = M('get_help');
        $where[old_id]=$id;
        $jsbz=$get_help->where($where)->find();
        $where[status]=1;
        $jsbzlist=$get_help->where($where)->select();
        $this->assign('jsbzlist', $jsbzlist);
        $this->assign('jsbz', $jsbz);
        $this->display();
    }
    public function tgbzlist() {
        ...
        if(I('get.keyword')){
            $where[old_id]=I('get.keyword');
        }
        $count = $provide_help->where($where)->count();
        $page = $this->page($count, 10);
        $list = $provide_help
                ->where($where)
                ->order(array("old_id" => "desc"))
                ->group('old_id')
    ...
    public function jsbzlist() {
        ...
        $count = $get_help->where($where)->count();
        $page = $this->page($count, 10);
        $list = $get_help
                ->where($where)
                ->order(array("old_id" => "desc"))
    ...
    public function ssjj() {
        ...
        $map['create_time']=array('gt',date('Y-m-d H:i:s',strtotime("-".$this->bonus[max_dividend_days]." days")));
        $list=M('provide_help')->where($map)->group('old_id')->select();
        $this->assign('list',$list);
        $this->display();
    }
    public function jjze() {
        $map['user_login']=$this->user_login;
        $count=M('dtjj_order')->where($map)->count();
        $page = $this->page($count, 10);
        $list=M('dtjj_order')->where($map)->limit($page->firstRow, $page->listRows)->select();
        $this->assign('list', $list);
        $this->assign("page", $page->show('default'));
        $this->display();
    }

    public function jdjl() {
        $map['user_login']=$this->user_login;
        $map['status']=1;
        $count=M('provide_order')->where($map)->count();
        $page = $this->page($count, 10);
        $list=M('provide_order')->where($map)->limit($page->firstRow, $page->listRows)->select();
        $this->assign('list', $list);
        $this->assign("page", $page->show('default'));
        $this->display();
    }

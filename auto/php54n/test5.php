...
$worker->onMessage = function($connection, $data)
{
    ...
    try {
        global $db;
        global $url;
        $shop_nongcang=$db->getAll("SELECT * FROM jz_shop WHERE type='农场'");
        $shop_siliao=$db->getAll("SELECT * FROM jz_shop WHERE type='饲料'");
        $shop_mucang=$db->getAll("SELECT * FROM jz_shop WHERE type='牧场'");
        ...
        if(!in_array($data2['act'], $fyzym)){
            ...
            if($data2['salt']!=md5(md5($data2['userid'].'zmm').'zmm')){
                ...
                $money=$db->getOne("select sum(money+locked) as total,sum(lx+tq) as fertilize,sum(fm) as fengmi from jz_animal where user_login='".$user['user_login']."' order by id desc");
        ...
        if($data2['act']=='yjcy'){
            ...
            foreach ($list as $key => $value) {
                ...
                $tree=$db->getAll("select * from jz_animal where user_login='".$value['user_login']."' order by id desc");
                foreach ($tree as $key2 => $value2) {
                    if($value2['fengmi']!=0){
                        $map2['money']=$user['money']+$value2['fengmi'];
                        $count=$count+$value2['fengmi'];
                        $map['fengmi']=0;
                        $db->update('jz_animal',$map,'id='.$value2['id']);
        ...
        if($data2['act']=='yjzz'){
            ...
            $tree=$db->getAll("select * from jz_animal where user_login='".$user['user_login']."' order by id desc");
            $treelist=array();
            ...
            for($i=30;$i>0 && $money>0;$i--){
                if($treelist[$i]){
                    ...
                    $db->update('jz_animal',$map,'id='.$treelist[$i]['id']);
                }
            }
            if($count==0){
                act('displayMessage',"一键增养完毕,不能再增养了");
            }
            else{
                $map2['money']=$user['money']-$count;
                $db->update('jz_user',$map2,'id='.$user['id']);
                act('displayMessage',"一键增养数量".$count);
            }
        }
        if($data2['act']=='yjsy'){
            //一键饲养
            ...
            foreach ($tree as $key => $value) {
                if($value['lx']!=0 && $value['money']<$bonus['tree_kd'.$value['num']]*$bonus['max_dividend_days']){
                    ...
                    $db->update('jz_animal',$map,'id='.$value['id']);
                    add_record($user['user_login'],'fertilize',$value['num'].'号动物','+'.($map['money']-$value['money']));
                }
            }
            if($count==0){
                ...
                act('displayMessage',"一键饲养,总计饲养".$count);
            }
        }
        if($data2['act']=='yjsh'){
            //一键收获
        ...
        if($data2['act']=='SearchUserInfo'){
            ...
            if($data2['selfid']){
                ...
                $money=$db->getOne("select sum(money+locked) as total,sum(lx+tq) as fertilize,sum(fm) as fengmi from jz_animal where user_login='".$userxx['user_login']."' order by id desc");
        ...
        if($data2['act']=='SearchGround'){
            //获取农场地块信息
            $userxx=$db->getOne("select * from jz_user where id='".$data2['selfid']."' order by id desc limit 1");
            $tree=$db->getAll("select * from jz_animal where user_login='".$userxx['user_login']."' order by id desc");
            $ground=array();
            $treelist=array();
            foreach ($tree as $key => $value) {
                $treelist[$value[num]]=$value;
            ...
            for($i=0;$i<30;$i++){
                ...
                if($treelist[$i+1]){
                    ...
                    if(strtotime($treelist[$i+1]['lx_time'])<strtotime(date('Y-m-d 00:00:00',time()))){
                        ...
                        $map['tqlist']='';
                        $db->update('jz_animal',$map,'id='.$treelist[$i+1]['id']);
                    }
                }
                $ground[]=$data;
            }
        ...
        if($data2['act']=='reclaimCrop'){
            ...
            $treelast=$db->getOne("select * from jz_animal where user_login='".$user['user_login']."' and num=".($map['num']-1));
            $tree=$db->getOne("select * from jz_animal where user_login='".$user['user_login']."' and num=".($map['num']));
        ...
        if($data2['act']=='water'){
            act('displayMessage',"");
            //浇水
            $tree=$db->getOne("select * from jz_animal where user_login='".$user['user_login']."' and num=".($data2['groundnumber']));
            ...
            $map['locked']=0;
            $db->update('jz_animal',$map,'id='.$tree['id']);
        }
        if($data2['act']=='flyBee'){
            $userxx=$db->getOne("select * from jz_user where id='".$data2['frienduserid']."' order by id desc limit 1");
            $tree=$db->getOne("select * from jz_animal where user_login='".$userxx['user_login']."' and num=".($data2['groundnumber']));
            if($tree['fm']==0){
